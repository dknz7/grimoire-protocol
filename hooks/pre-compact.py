"""
Grimoire Protocol — PreCompact hook.

Fires BEFORE Claude Code auto-compacts the context window. Captures
conversation transcript to inbox/sessions/ so knowledge isn't lost
to summarization during long sessions.

Same logic as session-end.py but with MIN_TURNS=5 (more conservative).

Setup:
  1. Copy this file to <vault>/scripts/grimoire/pre-compact.py
...
def main() -> None:
    ...
    filename = f"{timestamp}-{session_short}-precompact.md"
    ...
    logging.info(
        "Wrote pre-compact capture: %s (%d turns, %d chars)",
